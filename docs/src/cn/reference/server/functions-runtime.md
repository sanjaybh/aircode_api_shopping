...
::: tip 提示
在调试代码时，我们提供了[使用线上请求调试](/cn/guide/functions/debug#使用线上请求调试)功能，方便你快速复现线上问题。
:::
...
系统环境变量不支持修改，若希望设置自定义的环境变量，可参考[使用环境变量](/cn/guide/functions/env)。

## 自动扩缩容 {#auto-scaling}

AirCode 的云函数运行时会根据请求量自动扩缩容，无需开发者进行任何配置即可应对流量起伏。

处理函数请求时，会优先使用已有的可用实例。若当前实例满载，则会自动创建新实例来处理请求。当请求量下降后，闲置实例会被自动回收。关于实例伸缩的限制请参考[资源限制 - 云函数 - 实例伸缩](/cn/about/limits#functions-instance-scaling)。
...
进入「闲置」状态的时间和冷启动耗时等限制请参考[资源限制 - 云函数 - 冷启动](/cn/about/limits#functions-cold-start)。
